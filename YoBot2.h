					...
					if (Distance2D(unit->pos, game_info.enemy_start_locations.front()) < 15.0f) {
						...
						if (list.size() != 0) {
							...
							if (!list[target]->is_flying) {
								Actions()->UnitCommand(unit, ABILITY_ID::ATTACK_ATTACK, list[target]);
								continue;
							}
						}
			...
			for (auto & gw : Observation()->GetUnits(Unit::Alliance::Self, IsUnit(UNIT_TYPEID::PROTOSS_GATEWAY))) {
				...
				if (gw->orders.size() == 0) {
					...
					if (nexl.size() != 0) {
						...
						if (nexi->energy >= 50) {
							Actions()->UnitCommand(nexi, ABILITY_ID::EFFECT_CHRONOBOOST, gw);
						}
					}
					break;
				}				
	...
	bool TryBuildSupplyDepot() {
		...
		if (false && CountUnitType(UNIT_TYPEID::PROTOSS_PYLON) == 0) {
			...
			if (gws.size() != 0) {
				...
				Point2D candidate = Point2D(proxy.x + rx * 15.0f, proxy.y + ry * 15.0f);
				bool good = false;
				int iter = 0;
				while (!good && iter++ < 10){
